...
def event_identification(arena):        # NOTE: You can tweak this function in case you need to give more inputs 
    ...
    ADD YOUR CODE HERE
    '''
    img = cv.cvtColor(arena, cv.COLOR_BGR2GRAY)
 
    # Thresholding the image
    (thresh, img_bin) = cv.threshold(img, 254, 255,cv.THRESH_BINARY)
    # Invert the image
    img_bin = 255-img_bin
    ...
    for c in contours:
        ...
        if (w >= lb and h >= lb) and (w <= hb and h <= hb):
            idx += 1
            new_img = arena[y:y+h, x:x+w]
            img_path = dir_path+str(idx) + '.png'
            event_list.append(img_path)
